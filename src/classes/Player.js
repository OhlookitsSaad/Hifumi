...
class Player {
    ...
    initialize() {
        ...
        this.client.channels.fetch(channel_id)
            .then((channel) => {
                if (!channel.joinable) {
                    logger.fatal("Hifumi failed to connect to the configured Discord voice channel, maybe I don't have enough permissions?");
                    process.exit(1);
                }
            ...
            .catch((error) => {
                if (error === 'DiscordAPIError: Unknown Channel') {
                    logger.fatal('Hifumi failed to connect to the channel because the channel does not exist on Discord, please check the channel ID set up in your settings file.');
                } else {
                    logger.fatal('Something went wrong when trying to look for the channel Hifumi was supposed to join.', error);
                }
                process.exit(1);
            });
    }
