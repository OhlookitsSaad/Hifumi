...
const handleGuildCreate = (client, guild) => {
    ...
    if (numOfGuilds > 1) {
        logger.warn(`Joined the guild ${guild.name}. Hifumi is currently designed to work only on one server. Currently I'm connected to ${numOfGuilds} servers, unexpected behavior may occur.`);
    } else {
...
const handleGuildDelete = (client, guild) => {
    ...
    if (numOfGuilds > 1) {
        logger.warn(`Left the guild ${guild.name}. Hifumi is currently designed to work only on one server. Currently I'm connected to ${numOfGuilds} servers, unexpected behavior may occur.`);
    } else if (numOfGuilds === 0) {
        logger.warn(`Left the guild ${guild.name}! Hifumi has failed to connect to a Discord voice channel, please invite the bot to a guild and then try again.`);
    } else {
...
const handleReady = (player) => {
    logger.info('Hifumi has sucessfully connected to the Discord APi through WebSockets!- Ready.');
    player.initialize();
};
